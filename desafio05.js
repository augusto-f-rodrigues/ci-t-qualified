...
function calculaPorcentagemSorteio(assinante, minutosAssistidos) {
  let pessoasComHoras = assinante.map((el, i) => {
    return [el, minutosParaHoras(minutosAssistidos[i]), 1];
  });

  const assinantesComDobro = pessoasComHoras.map((el) => {
    if (el[0] == true) {
      return (el[1] *= 2);
    } else {
      return el[1];
  ...
  const totalDeHoras = assinantesComDobro.reduce((prev, cur) => {
    return prev + cur;
  });
  const baseParaPorcentagem = 100 / totalDeHoras;

  const resposta = assinantesComDobro.map((el) => {
    return Number((el * baseParaPorcentagem).toFixed());
  });
...
console.log(
  calculaPorcentagemSorteio(
    ...
    [6144, 2742, 330, 30, 1500, 4032, 24036, 3288, 2076, 24540, 4716]
  )
);
