...
function calculaNumeroDaSenha(senha) {
  let senhaBinaria = [];
  for (let i = 0; i <= 9; i++) {
    let quantidadeNumUm = senha
      .map((el) => {
        return el[i];
      })
      .filter((el) => el == 1).length;

    if (quantidadeNumUm >= 5) {
      senhaBinaria.push(1);
    } else {
      senhaBinaria.push(0);
    }
  ...
  senhaBinaria = Number(senhaBinaria.join(""));

  const senhaDecimal = parseInt(senhaBinaria, 2);

  return senhaDecimal;
}

const resultado = calculaNumeroDaSenha([
  "0110100000",
  "1001011111",
  "1110001010",
  "0111010101",
